...
def resolve_status_template(
    ...
    *,
    ...
    # Candidates fall into three classes. The .goc.json sidecar is the identity
    # authority; the visible name prefix is only a fallback signal for spotting a
    # generated save whose sidecar is missing, and never grants adoption.
    same_campaign: list[Path] = []
    foreign_generated: list[Path] = []
    ordinary: list[Path] = []
    errors: list[str] = []
    for candidate in candidates:
        try:
            ...
            continue
        if _binds_to_campaign(candidate, campaign_path):
            same_campaign.append(candidate)
        elif _has_sidecar(candidate) or _is_own_save(archive, candidate, name_prefix):
            # Generated by this tool but not bound to this campaign, or bound to
            # another one. Never adopted automatically, however few candidates
            # remain: its saveinfo belongs to a different strategic campaign.
            foreign_generated.append(candidate)
        else:
...
def _has_sidecar(candidate: Path) -> bool:
    """True when a save carries a Gates of CodeX export manifest beside it."""
    from .service import GatesOfCodeXService

    return GatesOfCodeXService.manifest_path(candidate).is_file()
