...
# instead of cloning purchase definitions into goc_active_actor_units.set.
CODEX_PASSTHROUGH_ACTORS = frozenset({"prc"})
...
def is_codex_passthrough_actor(actor_id: str) -> bool:
    return str(actor_id) in CODEX_PASSTHROUGH_ACTORS


def activation_mode_for_actor(actor_id: str) -> str:
    if is_codex_passthrough_actor(actor_id):
        return ACTIVATION_MODE_CODEX_PASSTHROUGH
    return ACTIVATION_MODE_EXPANDED


def manifest_activation_mode(manifest: Mapping[str, Any]) -> str:
    raw = str(manifest.get("activation_mode") or "").strip()
    if not raw:
        # Legacy manifests without the field are always full Expanded projections.
        return ACTIVATION_MODE_EXPANDED
    if raw not in {
        ACTIVATION_MODE_EXPANDED,
        ACTIVATION_MODE_CODEX_PASSTHROUGH,
    }:
        raise ExpandedNationsError(f"Unsupported activation mode: {raw}")
    return raw
