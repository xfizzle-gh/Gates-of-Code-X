...
def activate_actor_projection(
    ...
    *,
    ...
    Generated files exist only in the final Gates layer. Core Code:X is restored
    by removing the verified managed projection. Upstream mod trees are read but
    never modified.
    """
    ...
    outputs: dict[Path, bytes] = {
        ...
        RESEARCH_RELATIVE[side]: render_research_file(actor).encode("utf-8"),
    }
    ...
    manifest_payload = {
        ...
        "research_node_count": len(actor["research_nodes"]),
    ...
    install_projection(final_root, outputs, pretty_json(manifest_payload).encode("utf-8"))
