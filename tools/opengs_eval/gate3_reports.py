#!/usr/bin/env python3
"""Derive authoritative reports for isolated OpenGS Gate 3 candidates."""
...
def _json_bytes(data: bytes, label: str) -> Any:
    ...
    if canonical_json_bytes(value) != data:
        raise Gate3Error(f"{label} must be canonical UTF-8/LF JSON")
    return value
...
def derive_reports(
    ...
    gate1_manifest: Mapping[str, Any],
    ...
    candidate_summary = _validate_candidate_dataset(dataset, str(config["candidate_id"]))
    ...
    provenance = _report("gates-of-codex.opengs-gate3-provenance", {
        ...
        "projection": input_manifest["projection"],
        "lon_lat_bounds": input_manifest["lon_lat_bounds"],
        "projected_bounds": input_manifest["projected_bounds"],
        "dimensions": input_manifest["dimensions"],
        "theatre_mask": input_manifest["theatre_mask"],
        "gate1_mask_adapter": "authenticated_boundary_outside_color_v1",
        "gate1_manifest_sha256": file_sha256["gate1/run_manifest.json"],
        "gate2_adapter_manifest_sha256": file_sha256["candidate/adapter_manifest.json"],
        "production_authority_changed": False,
        "campaign_authority_generated": False,
    })
    count_report = _report("gates-of-codex.opengs-gate3-count-report", {
        "candidate_id": config["candidate_id"],
        "comparison_target_total": config["counts"]["comparison_target_total"],
        "requested": requested,
        "actual": actual,
        "gate2": candidate_summary,
        "source_component_counts": input_manifest["component_counts"],
        "delta_from_comparison_target": actual["provinces"] - config["counts"]["comparison_target_total"],
        "lake_overage_policy": "reported_not_silently_filtered",
    })
    topology_report = _report("gates-of-codex.opengs-gate3-topology-report", {
        "candidate_id": config["candidate_id"],
        "ok": topology.get("ok") is True,
        "province_count": topology.get("province_count"),
        "component_count": topology.get("component_count"),
        "hole_count": topology.get("hole_count"),
        "adjacency_edge_count": topology.get("adjacency_edge_count"),
        "border_class_counts": topology.get("border_class_counts"),
        "max_triangle_area_relative_error": topology.get("max_triangle_area_relative_error"),
        "minimum_shared_edge_pixels": topology.get("minimum_shared_edge_pixels"),
        "topology_audit_sha256": file_sha256["candidate/topology_audit.json"],
    })
    adjacency_report = _report("gates-of-codex.opengs-gate3-adjacency-report", {
        "candidate_id": config["candidate_id"],
        "edge_count": candidate_summary["edge_count"],
        "reciprocal": True,
        "corner_or_diagonal_only_edges": 0,
        "minimum_shared_edge_pixels": config["gate2"]["minimum_shared_edge_pixels"],
        "edge_pairs_sha256": sha256_bytes(canonical_json_bytes(dataset["edges"])),
    })
    ...
    terrain_report = _report("gates-of-codex.opengs-gate3-terrain-report", {
        ...
        "dominant_terrain_province_counts": dict(sorted(dominant.items())),
        "terrain_pixel_counts": dict(sorted(coverage_pixels.items())),
        "terrain_raster_sha256": file_sha256["inputs/terrain.png"],
        "limitation": "three-class physical baseline only; not production terrain authority",
    })
    ...
    water_report = _report("gates-of-codex.opengs-gate3-water-policy-report", {
        "candidate_id": config["candidate_id"],
        "policy": config["water_policy"],
        "water_count": len(water_provinces),
        "water_type_counts": candidate_summary["water_type_counts"],
    ...
    geography_report = _report("gates-of-codex.opengs-gate3-geography-report", {
        "candidate_id": config["candidate_id"],
        "theatre_policy": config["theatre"]["policy"],
        "lon_lat_bounds": input_manifest["lon_lat_bounds"],
        "projected_bounds": input_manifest["projected_bounds"],
        "projection": input_manifest["projection"],
        "dimensions": input_manifest["dimensions"],
        "theatre_mask": input_manifest["theatre_mask"],
        "feature_counts": input_manifest["feature_counts"],
    ...
    density_report = _report("gates-of-codex.opengs-gate3-density-report", {
        "candidate_id": config["candidate_id"],
        "policy": input_manifest["density"]["policy"],
        "statistics": {key: input_manifest["density"][key] for key in ("minimum", "maximum", "mean")},
        "density_raster_sha256": file_sha256["inputs/density.png"],
        "source_feature_counts": {key: input_manifest["feature_counts"][key] for key in ("populated_places", "boundary_line_parts", "river_line_parts")},
    })
    performance_report = _report("gates-of-codex.opengs-gate3-performance-report", {
        "candidate_id": config["candidate_id"],
        "dimensions": input_manifest["dimensions"],
        "province_count": candidate_summary["province_count"],
        "vertex_count": dataset_meta["vertex_count"],
        "triangle_count": dataset_meta["triangle_count"],
        "border_segment_count": dataset_meta["border_segment_count"],
    ...
    debug_manifest = _report("gates-of-codex.opengs-gate3-debug-map", {
        ...
        "candidate_manifest_path": "candidate/map_manifest.json",
        "candidate_manifest_sha256": file_sha256["candidate/map_manifest.json"],
        "debug_only": True,
        "default_map": False,
        "production_registration": False,
        "earth3_fallback_unchanged": True,
        "campaign_authority_generated": False,
    })
    return {
        ...
        "adjacency_report.json": adjacency_report,
        "terrain_report.json": terrain_report,
        "water_policy_report.json": water_report,
        "geography_report.json": geography_report,
        "density_report.json": density_report,
        "performance_report.json": performance_report,
        "debug_map_manifest.json": debug_manifest,
    }
