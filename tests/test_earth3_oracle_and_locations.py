...
class ThresholdDecisionConfigTests(unittest.TestCase):
    def test_threshold_decisions_file_schema(self) -> None:
        ...
        self.assertIn(data["owner_review_status"], {"pending_owner_visual_approval", "owner_approved_v7_europe_boundary", "owner_approved_v7_urals"})
    ...
    def test_masked_candidate_loads_frozen_overrides(self) -> None:
        ...
        # Murmansk/Kola approach is allowed in mask v6; Arkhangelsk included in v7.
...
class ExactLocationUnitTests(unittest.TestCase):
    def test_gating_key_set_complete(self) -> None:
        expected = {
            ...
            "Volgograd",
            "Vorkuta_siberia",
            "Yekaterinburg_east_urals",
            "Yerevan",
            "Zaporizhzhia",
        }
        self.assertEqual(set(GATING_LOCATION_KEYS), expected)
        self.assertEqual(len(GATING_LOCATION_KEYS), 57)
...
class CommittedAuditArtifactTests(unittest.TestCase):
    ...
    def test_boundary_review_covers_55(self) -> None:
        ...
        self.assertIn(data["status"], {"pending_owner_visual_approval", "owner_approved_v7_europe_boundary", "owner_approved_v7_urals"})
        for row in data["provinces"]:
            self.assertIn("owner_decision", row)
            self.assertIn(row["owner_review_status"], {"pending_owner_visual_approval", "owner_approved_v7_europe_boundary", "owner_approved_v7_urals"})
...
class LiveArchiveCorrectnessTests(unittest.TestCase):
    ...
    def test_exact_required_locations(self) -> None:
        ...
        self.assertEqual(report["gating_key_count"], 57)
